...
class EmotionAnalyzer:
    ...
    def get_summarized_result(self, paragraph: str) -> dict[str, float]:
        analyzed_result = self.analyze_paragraph(paragraph)

        summation = torch.sum(torch.stack(analyzed_result), dim=0)
        summation = softmax(summation, dim=1) * 100
        summation = summation.tolist()[0]
